# -*- coding: UTF8 -*-
from sigsis.settings import STATIC_ROOT
...
class Simulador(forms.Form):
    ...
    titulo = forms.CharField(label='Título do gráfico', max_length=100, widget=forms.TextInput(attrs={'required': True, 'tabindex':1}))
    ...
    t_start = forms.FloatField(label='Início do eixo [s]', widget=forms.NumberInput(attrs={'required': True, 'class':'input_menor', 'tabindex':4}))
    t_stop = forms.FloatField(label='Fim do eixo [s]', widget=forms.NumberInput(attrs={'required': True, 'class':'input_menor','tabindex':5}))
    # fieldset "Sinal"
    ...
    freq = forms.FloatField(label='Frequência Fundamental <i>f</i> [Hz]', min_value=1, widget=forms.NumberInput(attrs={'required': True, 'class':'input_menor', 'tabindex':9}))
    ...
    plot_quant_mr = forms.BooleanField(label='Sinal Quantizado <i>Mid-rise</i> [Magenta]', required=False, widget=forms.CheckboxInput(attrs={'tabindex':11}))
    plot_quant_eq_mt = forms.BooleanField(label='Erro de Quantização <i>Mid-tread</i> [Cinza]', required=False, widget=forms.CheckboxInput(attrs={'tabindex':11}))
